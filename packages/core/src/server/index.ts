...
// ─── createFrappeClient ───────────────────────────────────────────────────────
// Prisma-style typed client. Define your schema once, get fully-typed access
// to every DocType without repeating generic parameters everywhere.
//
// Usage:
//   // lib/db.ts
//   import { createFrappeClient } from '@frappe-next/core/server'
//   import type { Customer, SalesOrder } from '@/types/frappe-types'
//
//   export const db = createFrappeClient<{
//     Customer:      Customer
//     'Sales Order': SalesOrder
//   }>()
//
//   // In any Server Component:
//   const orders = await db['Sales Order'].list({ fields: ['name', 'status'] })
//   // → Pick<SalesOrder, 'name' | 'status'>[]
//
//   const customer = await db.Customer.get('CUST-0001')
//   // → Customer

/** Per-DocType API surface exposed by createFrappeClient. */
export interface FrappeDocClient<T> {
  /** Fetch a single document by name. */
  get(name: string, options?: FrappeFetchOptions): Promise<T>
  /** Fetch a document or null on 404 — no try/catch needed. */
  getOrNull(name: string, options?: FrappeFetchOptions): Promise<T | null>
  /** List with field selection — return type narrows to the selected fields. */
  list<const F extends ReadonlyArray<keyof T & string>>(
    args: Omit<GetListArgs, 'fields'> & { fields: F },
    options?: FrappeFetchOptions,
  ): Promise<Pick<T, F[number]>[]>
  /** List without field selection — returns full T[]. */
  list(args?: GetListArgs, options?: FrappeFetchOptions): Promise<T[]>
  /** Count documents matching the given filters. */
  count(filters?: readonly FrappeFilter[], options?: FrappeFetchOptions): Promise<number>
  /** Invalidate the Next.js ISR cache for a specific document. */
  revalidate(name: string): void
  /** Invalidate the Next.js ISR cache for the entire list. */
  revalidateList(): void
}

/** Full client type — one key per DocType in the schema. */
export type FrappeClient<Schema extends Record<string, object>> = {
  readonly [K in keyof Schema & string]: FrappeDocClient<Schema[K]>
}

/**
 * Create a fully-typed Frappe client from your DocType schema.
 *
 * Pass the schema as a type parameter (generated by `bench generate-types`).
 * Returns a proxy object — no runtime overhead beyond the property access.
 */
export function createFrappeClient<
  Schema extends Record<string, object>,
>(): FrappeClient<Schema> {
  return new Proxy({} as FrappeClient<Schema>, {
    get(_target, doctype: string) {
      // The Proxy get trap is inherently untyped at runtime — FrappeClient<Schema>
      // provides full type safety at every call site. The cast here is intentional.
      return {
        get:             (name: string, opts?: FrappeFetchOptions)          => getDoc(doctype, name, opts),
        getOrNull:       (name: string, opts?: FrappeFetchOptions)          => getDocOrNull(doctype, name, opts),
        list:            (args?: GetListArgs, opts?: FrappeFetchOptions)    => getList(doctype, args, opts),
        count:           (filters?: readonly FrappeFilter[], opts?: FrappeFetchOptions) =>
                           getCount(doctype, filters as FrappeFilter[], opts),
        revalidate:      (name: string)                                     => revalidateDoc(doctype, name),
        revalidateList:  ()                                                 => revalidateList(doctype),
      } as unknown as FrappeDocClient<Schema[keyof Schema & string]>
    },
  })
}
